...
## 3.1.2 Vaccination Series Complete
...
plot1 = ggplot(CasesRaceE, aes(x = Race.Ethnicity, y = Percent.of.cases, fill=Percent.of.cases)) + geom_bar(stat = "identity") + coord_flip() + ylab("Percent of Cases") + xlab("Race/Ethnicity") + scale_fill_continuous(name = "Percent of Cases") + theme(axis.title.x=element_blank(), axis.title.y=element_blank())

plot2 = ggplot(DeathsRaceE, aes(x = Race.Ethnicity, y = Percent.of.deaths, fill=Percent.of.deaths)) + geom_bar(stat = "identity") + coord_flip() + ylab("Percent of Deaths") + xlab("Race/Ethnicity") + scale_fill_continuous(name = "Percent of Deaths") + theme(axis.title.x=element_blank(), axis.title.y=element_blank())

grid.arrange(plot1, plot2)
```

Figure 3.2.1: This is a plot comparing and contrasting the cumulative percentage of cases and deaths between various races and ethnicities.

## 3.2.2 Sex

- This next plot describes and compares the percentage of cases against the percentage of deaths due to COVID in the US for the demographic of sex. For the category of sex described as "other", the original dataset gave non-numerical value of <0.1. For the sake of cohesiveness in the plot, such non-numerical value was rounded to 0.1 as mentioned. Those who identify as either male or female make up the majority while those who do not identify in either gender make up a statistically insignificant percentage. In the percentage of cases, women account for about 5 percent more than men. On the other hand, with regards to the percentage of deaths, men account for about 10 percent more than women. 
...
plot1_2 = ggplot(CasesSex, aes(x = Sex, y = Percent.of.cases, fill=Percent.of.cases)) + geom_bar(stat = "identity") + coord_flip() + ylab("Percent of Cases") + xlab("Sex") + scale_fill_continuous(name = "Percent of Cases") + theme(axis.title.x=element_blank(), axis.title.y=element_blank())

plot2_2 = ggplot(DeathsSex, aes(x = Sex, y = Percentage.of.deaths, fill=Percentage.of.deaths)) + geom_bar(stat = "identity") + coord_flip() + ylab("Percent of Deaths") + xlab("Sex") + scale_fill_continuous(name = "Percent of Deaths") + theme(axis.title.x=element_blank(), axis.title.y=element_blank())

grid.arrange(plot1_2, plot2_2)
```

Figure 3.2.2: This is a plot comparing and contrasting the cumulative percentage of cases and deaths between males, females, and other gender identities.

## 3.2.3 Age

- This next plot describes and compares the percent of cases against the percent of deaths due to COVID in the US for various age groups. It should be noted that in the plot of percentage of deaths per age groups, the age groups of 5-11 years, 12-15 years, and 16-17 years all had values less than 0.1 percent in the original dataset. For the sake of cohesiveness in the plot, all such values have been rounded to 0.1 as previously mentioned. At a high level view, it appears that the age groups most susceptible to contracting COVID were the range of ages from 18 to 64 years old, the highest of which being from 18 to 29 years old at about 23 percent. With regards to percentages of death, that same age group of 18 to 29 years old actaully accounted for about 2 percent, which is a sharp contrast. The age range most susceptible to a fatality were the ages from 50 and above, with the highest death percentages landing in the age group of 85+ years old at about 28 percent.
...
plot1_3 = ggplot(CasesAge, aes(x = Age.Group, y = Percent.of.cases, fill=Percent.of.cases)) + geom_bar(stat = "identity") + coord_flip() + ylab("Percent of Cases") + xlab("Age Group") + scale_fill_continuous(name = "Percent of Cases") + theme(axis.title.x=element_blank(), axis.title.y=element_blank())

plot2_3 = ggplot(DeathsAge, aes(x = Age.Group, y = Percentage.of.deaths, fill=Percentage.of.deaths)) + geom_bar(stat = "identity") + coord_flip() + ylab("Percent of Deaths") + xlab("Age Group") + scale_fill_continuous(name = "Percent of Deaths") + theme(axis.title.x=element_blank(), axis.title.y=element_blank())

grid.arrange(plot1_3, plot2_3)
```

Figure 3.2.1: This is a plot comparing and contrasting the cumulative percentage of cases and deaths between a large spectrum of age groups.
